...
use crate::{
    Address, lederdagError, EntryFunctionId, EventGuid, HashValue, HexEncodedBytes,
    ...
    pub fn from_transaction_onchain_data(
        ...
        if txn.version > latest_ledger_version {
            match txn.transaction {
                lederdag_types::transaction::Transaction::UserTransaction(txn) => {
...
pub struct TransactionOnChainData {
    ...
    pub transaction: lederdag_types::transaction::Transaction,
    /// Information about the transaction
    pub info: lederdag_types::transaction::TransactionInfo,
    /// Events emitted by the transaction
    pub events: Vec<ContractEvent>,
    /// The accumulator root hash at this version
    pub accumulator_root_hash: lederdag_crypto::HashValue,
    /// Final state of resources changed by the transaction
    pub changes: lederdag_types::write_set::WriteSet,
}

impl From<(TransactionWithProof, lederdag_crypto::HashValue)> for TransactionOnChainData {
    fn from((txn, accumulator_root_hash): (TransactionWithProof, lederdag_crypto::HashValue)) -> Self {
        Self {
            version: txn.version,
...
impl
    From<(
        u64,
        lederdag_types::transaction::Transaction,
        lederdag_types::transaction::TransactionInfo,
        Vec<ContractEvent>,
        lederdag_crypto::HashValue,
        lederdag_types::write_set::WriteSet,
    )> for TransactionOnChainData
{
    fn from(
...
/// Enum of the different types of transactions in lederdag
...
pub struct BlockMetadataTransaction {
    ...
    /// If some, it means the internal txn type is `lederdag_types::transaction::Transaction::BlockMetadataExt`.
    /// Otherwise, it is `lederdag_types::transaction::Transaction::BlockMetadata`.
    ///
...
impl
    From<(
        lederdag_types::validator_txn::ValidatorTransaction,
        TransactionInfo,
        Vec<Event>,
        u64,
    )> for ValidatorTransaction
{
    fn from(
        ...
        match txn {
            lederdag_types::validator_txn::ValidatorTransaction::DKGResult(dkg_transcript) => {
            ...
            lederdag_types::validator_txn::ValidatorTransaction::ObservedJWKUpdate(
...
/// A more API-friendly representation of the on-chain `lederdag_types::jwks::QuorumCertifiedUpdate`.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize, Object)]
...
/// A more API-friendly representation of the on-chain `lederdag_types::aggregate_signature::AggregateSignature`.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize, Object)]
...
/// A more API-friendly representation of the on-chain `lederdag_types::jwks::ProviderJWKs`.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize, Object)]
...
impl TryFrom<MultiEd25519Signature> for TransactionAuthenticator {
    ...
    fn try_from(value: MultiEd25519Signature) -> Result<Self, Self::Error> {
        ...
        Ok(TransactionAuthenticator::multi_ed25519(
            ...
            lederdag_crypto::multi_ed25519::MultiEd25519Signature::new_with_signatures_and_bitmap(
                ed25519_signatures,
