...
use lederdag_types::vm_status::StatusCode;
use poem_openapi::{Enum, Object};
use serde::{Deserialize, Serialize};
use std::fmt::Formatter;

/// This is the generic struct we use for all API errors, it contains a string
/// message and an lederdag API specific error code.
#[derive(Debug, Clone, Serialize, Deserialize, Object)]
pub struct lederdagError {
    /// A message describing the error
    pub message: String,
    pub error_code: lederdagErrorCode,
    /// A code providing VM error details when submitting transactions to the VM
    pub vm_error_code: Option<u64>,
}

impl std::fmt::Display for lederdagError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "Error({:?}): {:#}", self.error_code, self.message)
    }
}

impl std::error::Error for lederdagError {}
...
fn test_serialize_deserialize() {
    let with_code = lederdagError::new_with_vm_status(
        "Invalid transaction",
        lederdagErrorCode::VmError,
        lederdag_types::vm_status::StatusCode::UNKNOWN_MODULE,
    );
    let _: lederdagError = bcs::from_bytes(&bcs::to_bytes(&with_code).unwrap()).unwrap();
    let _: lederdagError = serde_json::from_str(&serde_json::to_string(&with_code).unwrap()).unwrap();

    let without_code =
        lederdagError::new_with_error_code("some message", lederdagErrorCode::MempoolIsFull);
    let _: lederdagError = bcs::from_bytes(&bcs::to_bytes(&without_code).unwrap()).unwrap();
    let _: lederdagError =
