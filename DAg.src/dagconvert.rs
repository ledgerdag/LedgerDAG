...
use lederdag_crypto::{hash::CryptoHash, HashValue};
use lederdag_logger::{sample, sample::SampleRate};
use lederdag_resource_viewer::lederdagValueAnnotator;
use lederdag_storage_interface::DbReader;
use lederdag_types::{
...
pub struct MoveConverter<'a, S> {
    inner: lederdagValueAnnotator<'a, S>,
    db: Arc<dyn DbReader>,
...
impl<'a, S: StateView> MoveConverter<'a, S> {
    pub fn new(
        ...
        Self {
            inner: lederdagValueAnnotator::new(inner),
            db,
            indexer_reader,
        }
    }
    ...
    pub fn is_resource_group(&self, tag: &StructTag) -> bool {
        if let Ok(Some(module)) = self.inner.view_module(&tag.module_id()) {
            if let Some(md) = lederdag_framework::get_metadata(&module.metadata) {
    ...
    pub fn try_into_onchain_transaction(
        ...
        use lederdag_types::transaction::Transaction::{
        ...
        Ok(match data.transaction {
            ...
            lederdag_types::transaction::Transaction::ValidatorTransaction(txn) => {
    ...
    pub fn into_transaction_info(
        &self,
        version: u64,
        info: &lederdag_types::transaction::TransactionInfo,
        accumulator_root_hash: HashValue,
        write_set: lederdag_types::write_set::WriteSet,
    ...
    pub fn try_into_transaction_payload(
        &self,
        payload: lederdag_types::transaction::TransactionPayload,
    ) -> Result<TransactionPayload> {
        use lederdag_types::transaction::TransactionPayload::*;
        let ret = match payload {
            ...
            Multisig(multisig) => {
                let transaction_payload = if let Some(payload) = multisig.transaction_payload {
                    match payload {
                        lederdag_types::transaction::MultisigTransactionPayload::EntryFunction(
                            entry_function,
                        ) => {
    ...
    pub fn try_into_write_set_payload(
        &self,
        payload: lederdag_types::transaction::WriteSetPayload,
    ) -> Result<WriteSetPayload> {
        use lederdag_types::transaction::WriteSetPayload::*;
        let ret = match payload {
    ...
    pub fn try_into_raw_transaction(
        ...
        Ok(RawTransaction::new(
            ...
            self.try_into_lederdag_core_transaction_payload(payload)?,
            max_gas_amount.into(),
            gas_unit_price.into(),
    ...
    pub fn try_into_lederdag_core_transaction_payload(
        &self,
        payload: TransactionPayload,
    ) -> Result<lederdag_types::transaction::TransactionPayload> {
        use lederdag_types::transaction::TransactionPayload as Target;
