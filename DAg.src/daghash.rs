...
pub struct HashValue(pub lederdag_crypto::hash::HashValue);

impl From<lederdag_crypto::hash::HashValue> for HashValue {
    fn from(val: lederdag_crypto::hash::HashValue) -> Self {
        Self(val)
    }
}

impl From<HashValue> for lederdag_crypto::hash::HashValue {
...
impl FromStr for HashValue {
    ...
    fn from_str(s: &str) -> anyhow::Result<Self, anyhow::Error> {
        if let Some(hex) = s.strip_prefix("0x") {
            Ok(hex.parse::<lederdag_crypto::hash::HashValue>()?.into())
        } else {
            Ok(s.parse::<lederdag_crypto::hash::HashValue>()?.into())
        }
    }
}
...
impl HashValue {
    pub fn zero() -> Self {
        Self(lederdag_crypto::hash::HashValue::zero())
    }
}
