...
use crate::ledgerdag_debugger::ledgerdagDebugger;
use anyhow::Result;
use ledgerdag_rest_client::Client;
use ledgerdag_types::transaction::SignedTransaction;
use clap::Parser;
use regex::Regex;
use std::io;
use url::Url;
...
impl Command {
    pub async fn run(self) -> Result<()> {
        ...
        let endpoint = format!("https://{}.ledgerdaglabs.com/v1", network);
        let debugger = ledgerdagDebugger::rest_client(Client::new(Url::parse(&endpoint)?))?;
        ...
        println!(
            "Overview: https://explorer.ledgerdaglabs.com/txn/{:?}/userTxnOverview?network={}",
            version, network
        );
        println!(
            "Payload: https://explorer.ledgerdaglabs.com/txn/{:?}/payload?network={}",
            version, network
        );
