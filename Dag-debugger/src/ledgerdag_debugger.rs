...
use ledgerdag_block_executor::txn_provider::{default::DefaultTxnProvider, TxnProvider};
use ledgerdag_gas_profiling::{GasProfiler, TransactionGasLog};
use ledgerdag_rest_client::Client;
use ledgerdag_types::{
...
use ledgerdag_validator_interface::{
    ledgerdagValidatorInterface, DBDebuggerInterface, DebuggerStateView, RestDebuggerInterface,
};
use ledgerdag_vm::{
    ledgerdag_vm::ledgerdagVMBlockExecutor, data_cache::AsMoveResolver, ledgerdagVM, VMBlockExecutor,
};
use ledgerdag_vm_environment::environment::ledgerdagEnvironment;
use ledgerdag_vm_logging::log_schema::AdapterLogSchema;
use ledgerdag_vm_types::{module_and_script_storage::AsledgerdagCodeStorage, output::VMOutput};
use itertools::Itertools;
use std::{path::Path, sync::Arc, time::Instant};

pub struct ledgerdagDebugger {
    debugger: Arc<dyn ledgerdagValidatorInterface + Send>,
}

impl ledgerdagDebugger {
    pub fn new(debugger: Arc<dyn ledgerdagValidatorInterface + Send>) -> Self {
        Self { debugger }
    }
    ...
    pub fn execute_transaction_at_version_with_gas_profiler(
        ...
        let env = ledgerdagEnvironment::new(&state_view);
        let vm = ledgerdagVM::new(env.clone(), &state_view);
        let resolver = state_view.as_move_resolver();
        let code_storage = state_view.as_ledgerdag_code_storage(env);
...
fn execute_block_no_limit(
    ...
    let executor = ledgerdagVMBlockExecutor::new();
    executor
