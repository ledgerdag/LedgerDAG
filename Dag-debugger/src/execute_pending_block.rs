...
use crate::{ledgerdag_debugger::ledgerdagDebugger, common::Opts};
use anyhow::Result;
use ledgerdag_crypto::HashValue;
use ledgerdag_logger::info;
...
impl Command {
    pub async fn run(self) -> Result<()> {
        let debugger = if let Some(rest_endpoint) = self.opts.target.rest_endpoint {
            ledgerdagDebugger::rest_client(Client::new(Url::parse(&rest_endpoint)?))?
        } else if let Some(db_path) = self.opts.target.db_path {
            ledgerdagDebugger::db(db_path)?
        } else {
        ...
        let user_txns = if let Some(block_rest_endpoint) = self.block_rest_endpoint {
            ...
            let cmd = ledgerdag_consensus::util::db_tool::Command {
