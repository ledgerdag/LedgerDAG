...
use ledgerdag_move_debugger::ledgerdag_debugger::ledgerdagDebugger;
use ledgerdag_rest_client::Client;
use ledgerdag_types::transaction::Transaction;
use ledgerdag_vm::ledgerdagVM;
...
async fn main() -> Result<()> {
    ...
    ledgerdag_logger::Logger::new().init();
    ledgerdagVM::set_concurrency_level_once(1);

    let debugger = match args.target {
        Target::Rest { endpoint } => {
            ledgerdagDebugger::rest_client(Client::new(Url::parse(&endpoint)?))?
        },
        Target::DB { path } => ledgerdagDebugger::db(path)?,
    };
