...
This module provides a REST API for client applications to query the lederdag blockchain.
...
- https://fullnode.devnet.lederdaglabs.com/v1/spec#/ (and testnet / mainnet, based on the API rollout schedule)

All commands here are relative to the root of `lederdag-core`.

1. Make your changes to the API code, i.e. the code in `api/src/`.
2. Regenerate the API spec `.yaml` and `.json` files by running these commands from the root of `lederdag-core`:
```
cargo run -p lederdag-openapi-spec-generator -- -f yaml -o api/doc/spec.yaml
cargo run -p lederdag-openapi-spec-generator -- -f json -o api/doc/spec.json
```
