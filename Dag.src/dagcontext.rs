...
use lederdag_api_types::{
    lederdagErrorCode, AsConverter, BcsBlock, GasEstimation, LedgerInfo, ResourceGroup,
    TransactionOnChainData,
};
use lederdag_config::config::{GasEstimationConfig, NodeConfig, RoleType};
use lederdag_crypto::HashValue;
use lederdag_gas_schedule::{lederdagGasParameters, FromOnChainGasSchedule};
use lederdag_logger::{error, info, Schema};
use lederdag_mempool::{MempoolClientRequest, MempoolClientSender, SubmissionStatus};
use lederdag_storage_interface::{
    state_store::state_view::db_state_view::{
        DbStateView, DbStateViewAtVersion, LatestDbStateCheckpointView,
    },
    lederdagDbError, DbReader, Order, MAX_REQUEST_LIMIT,
};
use lederdag_types::{
...
impl Context {
    ...
    pub fn latest_state_view_poem<E: InternalError>(
        ...
        self.db
            ...
            .map_err(|e| E::internal_with_code(e, lederdagErrorCode::InternalError, ledger_info))
    }
    ...
    pub fn state_view<E: StdApiError>(
        ...
        let state_view = self
            ...
            .map_err(|err| {
                E::internal_with_code(err, lederdagErrorCode::InternalError, &latest_ledger_info)
            })?;
    ...
    pub fn get_oldest_version_and_block_height<E: ServiceUnavailableError>(
        ...
        self.db
            ...
            .map_err(|e| E::service_unavailable_with_code_no_info(e, lederdagErrorCode::InternalError))
    }
    ...
    pub fn get_latest_storage_ledger_info<E: ServiceUnavailableError>(
        ...
        let ledger_info = self
            ...
            .map_err(|e| {
                E::service_unavailable_with_code_no_info(e, lederdagErrorCode::InternalError)
            })?;
        ...
        if let Some(indexer_reader) = self.indexer_reader.as_ref() {
            if indexer_reader.is_internal_indexer_enabled() {
                if let Some(mut latest_version) = indexer_reader
                    ...
                    .map_err(|err| {
                        E::service_unavailable_with_code_no_info(err, lederdagErrorCode::InternalError)
                    })?
                {
    ...
    pub fn get_block_timestamp<E: InternalError>(
        ...
        self.db
            ...
            .map_err(|err| E::internal_with_code(err, lederdagErrorCode::InternalError, ledger_info))
    }
    ...
    pub fn render_transactions_sequential<E: InternalError>(
        ...
        mut timestamp: u64,
    ) -> Result<Vec<lederdag_api_types::Transaction>, E> {
        if data.is_empty() {
            return Ok(vec![]);
        }
        ...
        let converter = state_view.as_converter(self.db.clone(), self.indexer_reader.clone());
        let txns: Vec<lederdag_api_types::Transaction> = data
            .into_iter()
            .map(|t| {
            ...
            .map_err(|err| {
                E::internal_with_code(err, lederdagErrorCode::InternalError, ledger_info)
            })?;
    ...
    pub fn get_account_transactions<E: NotFoundError + InternalError>(
        ...
        let txns_res = if !db_sharding_enabled(&self.node_config) {
            ...
            self.indexer_reader
                ...
                .map_err(|e| lederdagDbError::Other(e.to_string()))
        };
        let txns = txns_res
    ...
    pub fn get_gas_schedule<E: InternalError>(
        &self,
        ledger_info: &LedgerInfo,
    ) -> Result<(u64, lederdagGasParameters), E> {
        ...
        // Otherwise refresh the cache
        {
            ...
            let state_view = self
                ...
                .map_err(|e| {
                    E::internal_with_code(e, lederdagErrorCode::InternalError, ledger_info)
                })?;
            ...
            let gas_schedule_params = {
                let may_be_params =
                    GasScheduleV2::fetch_config(&state_view).and_then(|gas_schedule| {
                        ...
                        lederdagGasParameters::from_on_chain_gas_schedule(
                            &gas_schedule,
                            feature_version,
                        )
                        .ok()
                ...
                match may_be_params {
                    ...
                    None => GasSchedule::fetch_config(&state_view)
                        .and_then(|gas_schedule| {
                            let gas_schedule = gas_schedule.into_btree_map();
                            lederdagGasParameters::from_on_chain_gas_schedule(&gas_schedule, 0).ok()
                        })
                        .ok_or_else(|| {
                            E::internal_with_code(
...
pub struct GasScheduleCache {
    ...
    gas_schedule_params: Option<lederdagGasParameters>,
}
    ...
    E: InternalError + Send + 'static,
{
    tokio::task::spawn_blocking(func)
        .await
        .map_err(|err| E::internal_with_code_no_info(err, lederdagErrorCode::InternalError))?
}
