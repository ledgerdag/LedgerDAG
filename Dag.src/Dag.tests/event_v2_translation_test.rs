...
use lederdag_api_test_context::{current_function_name, TestContext};
use lederdag_crypto::{ed25519::Ed25519PrivateKey, SigningKey, ValidCryptoMaterial};
use lederdag_sdk::types::LocalAccount;
use lederdag_types::account_config::RotationProofChallenge;
...
async fn test_event_v2_translation_coin_deposit_event() {
    ...
    context
        .api_execute_lederdag_account_transfer(account2, account1.address(), 101)
        .await;
    ...
    let payload = json!({
        ...
        "type_arguments": ["0x1::lederdag_coin::lederdagCoin"],
        "arguments": [
    ...
    context
        .api_execute_lederdag_account_transfer(account2, account1.address(), 102)
        .await;
    ...
    let resp = context
        .gen_events_by_handle(
            &account1.address(),
            "0x1::coin::CoinStore%3C0x1::lederdag_coin::lederdagCoin%3E",
            "deposit_events",
        )
        .await;
    assert!(is_expected_event(resp.as_array().unwrap().last().unwrap()));
...
async fn test_event_v2_translation_account_coin_register_event() {
    ...
    let is_expected_event = |e: &Value| {
        matches_event_details(
            ...
            == format!("0x{}", hex::encode("lederdagCoin".to_string().as_bytes()))
    };

    // Transfer coins from account2 to account1, emitting V2 events as the feature is enabled
    context
        .api_execute_lederdag_account_transfer(account1, account2.address(), 102)
        .await;
