...
use lederdag_api_test_context::{current_function_name, pretty, TestContext};
use lederdag_crypto::ed25519::Ed25519Signature;
use lederdag_types::{
...
async fn simulate_lederdag_transfer(
        ...
        let req = warp::test::request()
            ...
            .json(&json!({
                ...
                "payload": {
                    ...
                    "function": "0x1::lederdag_account::transfer",
                    "type_arguments": [],
                    "arguments": [
...
async fn test_simulate_transaction_with_valid_signature() {
    ...
    let resp = simulate_lederdag_transfer(&mut context, true, SMALL_TRANSFER_AMOUNT, 400, false).await;
...
async fn test_simulate_transaction_with_not_valid_signature() {
    let mut context = new_test_context(current_function_name!());
    let resp = simulate_lederdag_transfer(&mut context, false, SMALL_TRANSFER_AMOUNT, 200, true).await;
...
async fn test_simulate_transaction_with_insufficient_balance() {
    let mut context = new_test_context(current_function_name!());
    let resp = simulate_lederdag_transfer(&mut context, false, LARGE_TRANSFER_AMOUNT, 200, true).await;
    assert!(!resp[0]["success"].as_bool().is_some_and(|v| v));
}
