...
use lederdag_config::config::{NodeConfig, ViewFilter, ViewFunctionId};
...
fn build_coin_balance_request(address: &AccountAddress) -> Value {
    json!({
        ...
        "type_arguments": vec!["0x1::lederdag_coin::lederdagCoin"],
    })
}
...
async fn test_simple_view_invalid() {
    ...
    let resp = context
        ...
        .post(
            "/view",
            json!({
                "function":"0x1::lederdag_account::assert_account_exists",
...
async fn test_view_tuple() {
    let mut context = new_test_context(current_function_name!());
    let payload = lederdag_stdlib::publish_module_source(
        "test_module",
        r#"
