...
use lederdag_sdk::{transaction_builder::lederdag_stdlib::lederdag_token_stdlib, types::LocalAccount};
...
async fn test_merkle_leaves_with_nft_transfer() {
    ...
    let collection_builder =
        ctx.transaction_factory()
            .payload(lederdag_token_stdlib::token_create_collection_script(
    ...
    let token_builder =
        ctx.transaction_factory()
            .payload(lederdag_token_stdlib::token_create_token_script(
                collection_name.clone(),
                token_name.clone(),
    ...
    let transfer_to_owner_txn = creator.sign_multi_agent_with_transaction_builder(
        vec![owner],
        ctx.transaction_factory()
            .payload(lederdag_token_stdlib::token_direct_transfer_script(
                creator.address(),
                collection_name.clone(),
                token_name.clone(),
                0,
