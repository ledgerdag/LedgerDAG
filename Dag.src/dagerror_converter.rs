...
use lederdag_api_types::{mime_types::JSON, lederdagError, lederdagErrorCode};
use poem::{
...
/// be a JSON representation of lederdagError. For our own code, this is exactly
...
fn build_error_response(error_string: String) -> Response {
    Json(lederdagError::new_with_error_code(
        error_string,
        lederdagErrorCode::WebFrameworkError,
    ))
    .into_response()
}
