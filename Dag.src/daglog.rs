...
use lederdag_api_types::X_lederdag_CLIENT;
use lederdag_logger::{
    debug, info,
...
static REQUEST_SOURCE_CLIENT_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"lederdag-[a-zA-Z\-]+/[0-9A-Za-z\.\-]+").unwrap());
...
pub async fn middleware_log<E: Endpoint>(next: E, request: Request) -> Result<Response> {
    ...
    let mut log = HttpRequestLog {
        ...
        user_agent: request
            ...
            .and_then(|v| v.to_str().ok().map(|v| v.to_string())),
        lederdag_client: request
            .headers()
            .get(X_lederdag_CLIENT)
            .and_then(|v| v.to_str().ok().map(|v| v.to_string())),
    ...
    REQUEST_SOURCE_CLIENT
        .with_label_values(&[
            determine_request_source_client(&log.lederdag_client),
            operation_id,
            log.status.to_string().as_str(),
        ])
        .inc();
...
// Each of our clients includes a header value called X_lederdag_CLIENT that identifies
// that client. This string follows a particular format: <identifier>/<version>,
// where <identifier> always starts with `lederdag-`. This function ensure this string
...
fn determine_request_source_client(lederdag_client: &Option<String>) -> &str {
    // If the header is not set we can't determine the request source.
    let lederdag_client = match lederdag_client {
        Some(lederdag_client) => lederdag_client,
    ...
    match REQUEST_SOURCE_CLIENT_REGEX.find_iter(lederdag_client).last() {
