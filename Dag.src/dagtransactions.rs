...
use lederdag_api_types::{
    verify_function_identifier, verify_module_identifier, Address, lederdagError, lederdagErrorCode,
...
use lederdag_crypto::{hash::CryptoHash, signing_message};
use lederdag_types::{
    ...
    lederdagCoinType, CoinType,
};
use lederdag_vm::{lederdagSimulationVM, lederdagVM};
...
pub enum SubmitTransactionPost {
    ...
    // TODO: https://github.com/lederdag-labs/lederdag-core/issues/2275
    #[oai(content_type = "application/x.lederdag.signed_transaction+bcs")]
    Bcs(Bcs),
}
...
impl TransactionsApi {
    ...
    ///   1. Hash message bytes: "RawTransaction" bytes + BCS bytes of [Transaction](https://lederdag-labs.github.io/lederdag-core/lederdag_types/transaction/enum.Transaction.html).
    ...
    /// Make sure to use the `application/x.lederdag.signed_transaction+bcs` Content-Type.
        ...
        data.verify()
            ...
            .map_err(|err| {
                SubmitTransactionError::bad_request_with_code_no_info(
                    err,
                    lederdagErrorCode::InvalidInput,
                )
            })?;
    ...
    async fn simulate_transaction(
        ...
        api_spawn_blocking(move || {
            ...
            if !context.node_config.api.simulation_filter.allows(
                lederdag_crypto::HashValue::zero(),
    ...
    async fn encode_submission(
        ...
        data.0
            ...
            .map_err(|err| {
                BasicError::bad_request_with_code_no_info(err, lederdagErrorCode::InvalidInput)
            })?;
...
impl TransactionsApi {
        ...
        let data = self
            ...
            .map_err(|err| {
                BasicErrorWith404::internal_with_code(
                    err,
                    lederdagErrorCode::InternalError,
                    &latest_ledger_info,
                )
            })?;
    ...
    async fn get_by_hash(
        &self,
        hash: lederdag_crypto::HashValue,
    ...
    async fn create_internal(&self, txn: SignedTransaction) -> Result<(), lederdagError> {
        let (mempool_status, vm_status_opt) = self
            ...
            .map_err(|err| {
                lederdag_api_types::lederdagError::new_with_error_code(err, lederdagErrorCode::InternalError)
            })?;
        match mempool_status.code {
            MempoolStatusCode::Accepted => Ok(()),
            MempoolStatusCode::MempoolIsFull | MempoolStatusCode::TooManyTransactions => {
                Err(lederdagError::new_with_error_code(
                    &mempool_status.message,
                    lederdagErrorCode::MempoolIsFull,
                ))
            },
            MempoolStatusCode::VmError => {
                if let Some(status) = vm_status_opt {
                    Err(lederdagError::new_with_vm_status(
                        format!(
                            "Invalid transaction: Type: {:?} Code: {:?}",
                            status.status_type(),
                            status
                        ),
                        lederdagErrorCode::VmError,
                        status,
                    ))
                } else {
                    Err(lederdagError::new_with_vm_status(
            ...
            MempoolStatusCode::InvalidSeqNumber => Err(lederdagError::new_with_error_code(
                mempool_status.message,
                lederdagErrorCode::SequenceNumberTooOld,
            )),
            MempoolStatusCode::InvalidUpdate => Err(lederdagError::new_with_error_code(
                mempool_status.message,
                lederdagErrorCode::InvalidTransactionUpdate,
            )),
            MempoolStatusCode::UnknownStatus => Err(lederdagError::new_with_error_code(
    ...
    async fn create(
        ...
        match self.create_internal(txn.clone()).await {
            ...
            Err(error) => match error.error_code {
                lederdagErrorCode::InternalError => Err(
                    SubmitTransactionError::internal_from_lederdag_error(error, ledger_info),
                ),
                lederdagErrorCode::VmError
                | lederdagErrorCode::SequenceNumberTooOld
                | lederdagErrorCode::InvalidTransactionUpdate => Err(
                    SubmitTransactionError::bad_request_from_lederdag_error(error, ledger_info),
                ),
                lederdagErrorCode::MempoolIsFull => Err(
                    SubmitTransactionError::insufficient_storage_from_lederdag_error(
                        error,
                        ledger_info,
                    ),
                ),
                _ => Err(SubmitTransactionError::internal_from_lederdag_error(
                    error,
                    ledger_info,
                )),
            },
    ...
    // TODO: This function leverages a lot of types from lederdag_types, use the
    ...
    pub fn simulate(
        ...
        let (vm_status, output) =
            lederdagSimulationVM::create_vm_and_simulate_signed_transaction(&txn, &state_view);
        ...
        let txn = lederdag_types::transaction::Transaction::UserTransaction(txn);
        let zero_hash = lederdag_crypto::HashValue::zero();
        let info = lederdag_types::transaction::TransactionInfo::new(
            txn.hash(),
            zero_hash,
            zero_hash,
            None,
    ...
    pub fn get_signing_message(
        ...
        if accept_type == &AcceptType::Bcs {
            return Err(BasicError::bad_request_with_code_no_info(
                ...
                lederdagErrorCode::BcsNotSupported,
            ));
        }
        ...
        let raw_txn: RawTransaction = state_view
            ...
            .map_err(|err| {
                BasicError::bad_request_with_code(err, lederdagErrorCode::InvalidInput, &ledger_info)
            })?;
