...
//! The lederdag API response / error handling philosophy.
//!
...
// TODO: https://github.com/lederdag-labs/lederdag-core/issues/2279

use super::{accept_type::AcceptType, bcs_payload::Bcs};
use lederdag_api_types::{Address, lederdagError, lederdagErrorCode, HashValue, LedgerInfo};
use move_core_types::{
...
pub enum lederdagResponseContent<T: ToJSON + Send + Sync> {
...
/// an error response to manipulate the lederdagError inside it.
pub trait lederdagErrorResponse {
    fn inner_mut(&mut self) -> &mut lederdagError;
}
...
/// implementor also implements lederdagErrorResponse, which saves functions from
...
macro_rules! generate_error_traits {
    ($($trait_name:ident),*) => {
        paste::paste! {
        $(
        pub trait [<$trait_name Error>]: lederdagErrorResponse {
            // With ledger info and an error code
            #[allow(unused)]
            fn [<$trait_name:snake _with_code>]<Err: std::fmt::Display>(
                err: Err,
                error_code: lederdag_api_types::lederdagErrorCode,
                ledger_info: &lederdag_api_types::LedgerInfo,
            ...
            fn [<$trait_name:snake _with_vm_status>]<Err: std::fmt::Display>(
                err: Err,
                error_code: lederdag_api_types::lederdagErrorCode,
                vm_status: lederdag_types::vm_status::StatusCode,
                ledger_info: &lederdag_api_types::LedgerInfo
            ) -> Self where Self: Sized;

            #[allow(unused)]
            fn [<$trait_name:snake _from_lederdag_error>](
                lederdag_error: lederdag_api_types::lederdagError,
                ledger_info: &lederdag_api_types::LedgerInfo
            ) -> Self where Self: Sized;
        }
        )*
...
macro_rules! generate_error_response {
    ($enum_name:ident, $(($status:literal, $name:ident)),*) => {
        ...
        pub enum $enum_name {
            $(
            #[oai(status = $status)]
            $name(poem_openapi::payload::Json<Box<lederdag_api_types::lederdagError>>,
                // We use just regular u64 here instead of U64 since all header
                // values are implicitly strings anyway.
                /// Chain ID of the current chain
                #[oai(header = "X-lederdag-Chain-Id")] Option<u8>,
                /// Current ledger version of the chain
                #[oai(header = "X-lederdag-Ledger-Version")] Option<u64>,
                /// Oldest non-pruned ledger version of the chain
                #[oai(header = "X-lederdag-Ledger-Oldest-Version")] Option<u64>,
                /// Current timestamp of the chain
                #[oai(header = "X-lederdag-Ledger-TimestampUsec")] Option<u64>,
                /// Current epoch of the chain
                #[oai(header = "X-lederdag-Epoch")] Option<u64>,
                /// Current block height of the chain
                #[oai(header = "X-lederdag-Block-Height")] Option<u64>,
                /// Oldest non-pruned block height of the chain
                #[oai(header = "X-lederdag-Oldest-Block-Height")] Option<u64>,
                /// The cost of the call in terms of gas
                #[oai(header = "X-lederdag-Gas-Used")] Option<u64>,
            ),
            )*
        }
        ...
        impl $crate::response::[<$name Error>] for $enum_name {
            fn [<$name:snake _with_code>]<Err: std::fmt::Display>(
                ...
                let error = lederdag_api_types::lederdagError::new_with_error_code(err, error_code);
            ...
            fn [<$name:snake _with_vm_status>]<Err: std::fmt::Display>(
                ...
                let error = lederdag_api_types::lederdagError::new_with_vm_status(err, error_code, vm_status);
                let payload = poem_openapi::payload::Json(Box::new(error));
                Self::from($enum_name::$name(
                    payload,
                    Some(ledger_info.chain_id),
            ...
            fn [<$name:snake _from_lederdag_error>](
                lederdag_error: lederdag_api_types::lederdagError,
                ledger_info: &lederdag_api_types::LedgerInfo
            ) -> Self where Self: Sized {
                let payload = poem_openapi::payload::Json(Box::new(lederdag_error));
                Self::from($enum_name::$name(
                    payload,
                    Some(ledger_info.chain_id),
                    Some(ledger_info.ledger_version.into()),
        ...
        // Generate a function that helps get the lederdagError within.
        impl $crate::response::lederdagErrorResponse for $enum_name {
            fn inner_mut(&mut self) -> &mut lederdag_api_types::lederdagError {
                match self {
                    $(
    ...
    ($enum_name:ident, $(($status:literal, $name:ident)),*) => {
        ...
        pub enum $enum_name<T: poem_openapi::types::ToJSON + Send + Sync> {
            ...
            $name(
                // We use just regular u64 here instead of U64 since all header
                // values are implicitly strings anyway.
                $crate::response::lederdagResponseContent<T>,
                /// Chain ID of the current chain
                #[oai(header = "X-lederdag-Chain-Id")] u8,
                /// Current ledger version of the chain
                #[oai(header = "X-lederdag-Ledger-Version")] u64,
                /// Oldest non-pruned ledger version of the chain
                #[oai(header = "X-lederdag-Ledger-Oldest-Version")] u64,
                /// Current timestamp of the chain
                #[oai(header = "X-lederdag-Ledger-TimestampUsec")] u64,
                /// Current epoch of the chain
                #[oai(header = "X-lederdag-Epoch")] u64,
                /// Current block height of the chain
                #[oai(header = "X-lederdag-Block-Height")] u64,
                /// Oldest non-pruned block height of the chain
                #[oai(header = "X-lederdag-Oldest-Block-Height")] u64,
                /// The cost of the call in terms of gas
                #[oai(header = "X-lederdag-Gas-Used")] Option<u64>,
                ...
                #[oai(header = "X-lederdag-Cursor")] Option<String>,
            ),
            )*
        }
        ...
        // Generate a From impl that builds a response from lederdagResponseContent.
        ...
        impl <T: poem_openapi::types::ToJSON + Send + Sync> From<($crate::response::lederdagResponseContent<T>, &lederdag_api_types::LedgerInfo, [<$enum_name Status>])>
            for $enum_name<T>
        {
            fn from(
                (value, ledger_info, status): (
                    $crate::response::lederdagResponseContent<T>,
                    &lederdag_api_types::LedgerInfo,
                    [<$enum_name Status>]
                ),
            ) -> Self {
                match status {
        ...
        impl<T: poem_openapi::types::ToJSON + Send + Sync> From<(poem_openapi::payload::Json<T>, &lederdag_api_types::LedgerInfo, [<$enum_name Status>])>
            for $enum_name<T>
        {
            fn from(
                (value, ledger_info, status): (poem_openapi::payload::Json<T>, &lederdag_api_types::LedgerInfo, [<$enum_name Status>]),
            ) -> Self {
                let content = $crate::response::lederdagResponseContent::Json(value);
                Self::from((content, ledger_info, status))
            }
        }

        // Generate a From impl that builds a response from a Bcs<Vec<u8>> and friends.
        impl<T: poem_openapi::types::ToJSON + Send + Sync> From<($crate::bcs_payload::Bcs, &lederdag_api_types::LedgerInfo, [<$enum_name Status>])>
            for $enum_name<T>
        {
            fn from(
                (value, ledger_info, status): (
                ...
                let content = $crate::response::lederdagResponseContent::Bcs(value);
                Self::from((content, ledger_info, status))
            }
        }
        ...
        impl<T: poem_openapi::types::ToJSON + Send + Sync + serde::Serialize> $enum_name<T> {
                        ...
                        $crate::bcs_payload::Bcs(
                            bcs::to_bytes(&value)
                                .map_err(|e| E::internal_with_code(
                                    e,
                                    lederdag_api_types::lederdagErrorCode::InternalError,
                                    ledger_info
                                ))?
                        ),
                        ledger_info,
           ...
           pub fn try_from_json<E: $crate::response::InternalError>(
            ...
            pub fn with_cursor(mut self, new_cursor: Option<lederdag_types::state_store::state_key::StateKey>) -> Self {
                match self {
                    $(
                    [<$enum_name>]::$name(_, _, _, _, _, _, _, _, _, ref mut cursor) => {
                        *cursor = new_cursor.map(|c| lederdag_api_types::StateKeyWrapper::from(c).to_string());
                    }
                    )*
                }
                self
...
pub fn version_not_found<E: NotFoundError>(ledger_version: u64, ledger_info: &LedgerInfo) -> E {
    build_not_found(
        ...
        lederdagErrorCode::VersionNotFound,
        ledger_info,
    )
}
...
pub fn transaction_not_found_by_version<E: NotFoundError>(
    ...
    build_not_found(
        "Transaction",
        format!("Ledger version({})", ledger_version),
        lederdagErrorCode::TransactionNotFound,
        ledger_info,
    )
}
...
pub fn version_pruned<E: GoneError>(ledger_version: u64, ledger_info: &LedgerInfo) -> E {
    E::gone_with_code(
        ...
        lederdagErrorCode::VersionPruned,
        ledger_info,
    )
}
...
pub fn account_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        lederdagErrorCode::AccountNotFound,
        ledger_info,
    )
}
...
pub fn resource_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        lederdagErrorCode::ResourceNotFound,
        ledger_info,
    )
}
...
pub fn module_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        lederdagErrorCode::ModuleNotFound,
        ledger_info,
    )
}
...
pub fn struct_field_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        lederdagErrorCode::StructFieldNotFound,
        ledger_info,
    )
}
...
pub fn table_item_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        lederdagErrorCode::TableItemNotFound,
        ledger_info,
    )
}
...
pub fn block_not_found_by_height<E: NotFoundError>(
    ...
    build_not_found(
        ...
        lederdagErrorCode::BlockNotFound,
        ledger_info,
    )
}
