...
use lederdag_api_types::{
    verify_field_identifier, Address, lederdagErrorCode, AsConverter, IdentifierWrapper, LedgerInfo,
    MoveStructTag, VerifyInputWithRecursion, VersionedEvent, U64,
};
use lederdag_types::event::EventKey;
use poem_openapi::{
    param::{Path, Query},
    OpenApi,
};
...
impl EventsApi {
    ...
    async fn get_events_by_creation_number(
        &self,
        accept_type: AcceptType,
        /// Hex-encoded 32 byte lederdag account, with or without a `0x` prefix, for
