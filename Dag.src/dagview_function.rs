...
use lederdag_api_types::{
    lederdagErrorCode, AsConverter, MoveValue, ViewFunction, ViewRequest, MAX_RECURSIVE_TYPES_ALLOWED,
    U64,
};
use lederdag_bcs_utils::serialize_uleb128;
use lederdag_vm::lederdagVM;
...
pub enum ViewFunctionRequest {
    ...
    #[oai(content_type = "application/x.lederdag.view_function+bcs")]
    Bcs(Bcs),
}
...
impl ViewFunctionApi {
    ...
    ///
    /// The lederdag nodes prune account state history, via a configurable time window.
...
fn view_request(
    ...
    let values = output.values.map_err(|err| {
        BasicErrorWith404::bad_request_with_code_no_info(err, lederdagErrorCode::InvalidInput)
    })?;
