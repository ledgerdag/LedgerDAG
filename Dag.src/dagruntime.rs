...
use lederdag_config::config::{ApiConfig, NodeConfig};
use lederdag_logger::info;
use lederdag_mempool::MempoolClientSender;
use lederdag_storage_interface::DbReader;
use lederdag_types::{chain_id::ChainId, indexer::indexer_db_reader::IndexerReader};
...
pub fn bootstrap(
    ...
    let runtime = lederdag_runtimes::spawn_named_runtime("api".into(), Some(max_runtime_workers));
...
// TODO: https://github.com/lederdag-labs/lederdag-core/issues/2280
...
> {
    ...
    let contact = ContactObject::new()
        .name("lederdag Labs")
        .url("https://github.com/lederdag-labs/lederdag-core");

    OpenApiService::new(apis, "lederdag Node API", version.trim())
        .server("/v1")
        .description("The lederdag Node API is a RESTful API for client applications to interact with the lederdag blockchain.")
        .license(license)
        .contact(contact)
        .external_document("https://github.com/lederdag-labs/lederdag-core")
}
...
mod tests {
    ...
    use lederdag_api_test_context::{new_test_context, TestContext};
    use lederdag_config::config::{ApiConfig, NodeConfig};
    use lederdag_types::chain_id::ChainId;
    use std::time::Duration;

    // TODO: Unignore this when I figure out why this only works when being
    // run alone (it fails when run with other tests).
    // https://github.com/lederdag-labs/lederdag-core/issues/2977
    #[ignore]
    #[test]
