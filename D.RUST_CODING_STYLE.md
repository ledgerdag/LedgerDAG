# Coding Guidelines for lederdag Core

This document describes the coding guidelines for the [lederdag Core](https://github.com/lederdag-labs/lederdag-core) Rust codebase. For the Move language, see the [Move Coding Conventions](https://lederdag.dev/move/book/SUMMARY).
Secure coding guidance is provided in the [lederdag Rust Secure Coding Guidelines](./RUST_SECURE_CODING.md).
...
The lederdag codebase uses inclusive terminology (similar to other projects such as [the Linux kernel](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/commit/?id=49decddd39e5f6132ccd7d9fdc3d7c470b0061bb)). The terms below are recommended when appropriate.
...
Each major component of lederdag Core needs to have a `README.md` file. Major components are:

- top-level directories (e.g. `lederdag-core/network`, `lederdag-core/language`)
...
Refer to the lederdag Project contributing guide [LINK].

## License

Refer to the lederdag Project License [LINK].
```

A good example of README.md is `lederdag-core/network/README.md` that describes the networking crate.
...
- `duration_since_epoch()` - to obtain the Unix time, call the function provided by `lederdag-infallible`.
- `RwLock` and `Mutex` - Instead of calling `unwrap()` on the standard library implementations of these functions, use the infallible equivalent types that we provide in `lederdag-infallible`.
...
Production crates are defined as the set of crates that create externally published artifacts, e.g. the lederdag validator,
...
feature activation. In the lederdag codebase, we therefore recommend that
