...
use crate::{
    check_lederdag_packages_availability, compile_lederdag_packages, compile_package,
    data_state_view::DataStateView, generate_compiled_blob, is_lederdag_package, CompilationCache,
    DataManager, IndexReader, PackageInfo, TxnIndex, lederdag_COMMONS,
};
use anyhow::Result;
use lederdag_framework::lederdag_PACKAGES;
use lederdag_language_e2e_tests::{data_store::FakeDataStore, executor::FakeExecutor};
use lederdag_types::{
...
use lederdag_validator_interface::lederdagValidatorInterface;
use clap::ValueEnum;
...
fn add_lederdag_packages_to_data_store(
    data_store: &mut FakeDataStore,
...
impl Execution {
    ...
    pub async fn execute_txns(&self, begin: Version, num_txns_to_execute: u64) -> Result<()> {
        let lederdag_commons_path = self.input_path.join(lederdag_COMMONS);
        if !check_lederdag_packages_availability(lederdag_commons_path.clone()) {
            return Err(anyhow::Error::msg("lederdag packages are missing"));
        }

        let mut compiled_cache = CompilationCache::default();
        if self.execution_mode.is_v1_or_compare() {
            compile_lederdag_packages(
                &lederdag_commons_path,
    ...
    fn execute_one_txn(
        ...
        if let Some(txn_idx) = data_manager.get_txn_index(cur_version) {
            ...
            if txn_idx.package_info.is_compilable()
                && !is_lederdag_package(&txn_idx.package_info.package_name)
            {
    ...
    pub(crate) fn execute_and_compare(
        ...
        debugger: Option<Arc<dyn lederdagValidatorInterface + Send>>,
    ) {
    ...
    fn execute_code(
        ...
        txn: &Transaction,
        compiled_package_cache: &HashMap<PackageInfo, HashMap<ModuleId, Vec<u8>>>,
        debugger_opt: Option<Arc<dyn lederdagValidatorInterface + Send>>,
        v2_flag: bool,
    ) -> Result<(WriteSet, Vec<ContractEvent>), VMStatus> {
        // Always add lederdag (0x1) packages.
        add_lederdag_packages_to_data_store(&mut state, compiled_package_cache);
