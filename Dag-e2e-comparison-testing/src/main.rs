...
use anyhow::Result;
use lederdag_comparison_testing::{
    prepare_lederdag_packages, DataCollection, Execution, ExecutionMode, OnlineExecutor, lederdag_COMMONS,
};
...
pub enum Cmd {
    ...
    Dump {
        /// Endpoint url to obtain the txn data, e.g. `https://api.mainnet.lederdaglabs.com/v1` for mainnet.
        /// To avoid rate limiting, users need to apply for API key from `https://developers.lederdaglabs.com/`
    ...
    Online {
        ...
        /// e.g. `https://api.mainnet.lederdaglabs.com/v1` for mainnet.
        /// To avoid rate limiting, users need to apply for API key from `https://developers.lederdaglabs.com/`
        /// and set the env variable X_API_KEY using the obtained key
        endpoint: String,
        /// Path to the dumped data
...
async fn main() -> Result<()> {
    ...
    match args.cmd {
        Cmd::Dump {
            ...
            if !skip_source_code {
                prepare_lederdag_packages(output.join(lederdag_COMMONS)).await;
            }
        ...
        Cmd::Execute {
            ...
            prepare_lederdag_packages(input.join(lederdag_COMMONS)).await;
