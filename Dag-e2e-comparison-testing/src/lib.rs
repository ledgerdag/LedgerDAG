...
use lederdag_framework::{
    natives::code::PackageMetadata, unzip_metadata_str, BuiltPackage, lederdag_PACKAGES,
};
use lederdag_language_e2e_tests::data_store::FakeDataStore;
use lederdag_types::{
...
const lederdag_PACKAGES_DIR_NAMES: [&str; 5] = [
    "lederdag-framework",
    "move-stdlib",
    "lederdag-stdlib",
    "lederdag-token",
    "lederdag-token-objects",
];
...
pub const lederdag_COMMONS: &str = "lederdag-commons";
...
fn is_lederdag_package(package_name: &str) -> bool {
    lederdag_PACKAGES.contains(&package_name)
}

fn get_lederdag_dir(package_name: &str) -> Option<&str> {
    if is_lederdag_package(package_name) {
        for i in 0..lederdag_PACKAGES.len() {
            if lederdag_PACKAGES[i] == package_name {
                return Some(lederdag_PACKAGES_DIR_NAMES[i]);
            }
        }
    }
    None
...
async fn download_lederdag_packages(path: &Path) -> anyhow::Result<()> {
    let git_url = "https://github.com/lederdag-labs/lederdag-core";
    let tmp_dir = TempDir::new()?;
    Command::new("git")
        .args(["clone", git_url, tmp_dir.path().to_str().unwrap()])
        .output()
        .map_err(|_| anyhow::anyhow!("Failed to clone Git repository"))?;
    let source_framework_path = PathBuf::from(tmp_dir.path()).join("lederdag-move/framework");
    for package_name in lederdag_PACKAGES {
        let source_framework_path =
            source_framework_path.join(get_lederdag_dir(package_name).unwrap());
        let target_framework_path = PathBuf::from(path).join(get_lederdag_dir(package_name).unwrap());
        Command::new("cp")
            .arg("-r")
...
fn check_lederdag_packages_availability(path: PathBuf) -> bool {
    if !path.exists() {
        return false;
    }
    for package in lederdag_PACKAGES {
        if !path.join(get_lederdag_dir(package).unwrap()).exists() {
            return false;
        }
    }
    true
...
pub async fn prepare_lederdag_packages(path: PathBuf) {
    ...
    if success {
        ...
        download_lederdag_packages(&path).await.unwrap();
    }
}
...
fn compile_lederdag_packages(
    lederdag_commons_path: &Path,
    compiled_package_map: &mut HashMap<PackageInfo, HashMap<ModuleId, Vec<u8>>>,
    v2_flag: bool,
) -> anyhow::Result<()> {
    for package in lederdag_PACKAGES {
        let root_package_dir = lederdag_commons_path.join(get_lederdag_dir(package).unwrap());
        let compiler_version = if v2_flag {
            Some(CompilerVersion::latest_stable())
        } else {
            Some(CompilerVersion::V1)
        };
        // For simplicity, all packages including lederdag token are stored under 0x1 in the map
...
fn compile_package(
    ...
    let mut build_options = lederdag_framework::BuildOptions {
    ...
    for manifest_dep in manifest_deps {
        ...
        for pack_dep in &root_package_metadata.deps {
            ...
            if pack_dep_name == manifest_dep_name {
                if is_lederdag_package(&pack_dep_name) {
                    fix_manifest_dep(
                        dep,
                        &format!(
                            "{}/{}",
                            lederdag_COMMONS,
                            get_lederdag_dir(&pack_dep_name).unwrap()
                        ),
                    );
                    break;
                }
